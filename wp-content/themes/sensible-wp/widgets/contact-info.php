...
// constructor
    ...
    function __construct() {  
		...
		$widget_ops = array('classname' => 'sensiblewp_contact_info_widget', 'description' => esc_html__( 'Display your contact info', 'sensible-wp') );

        parent::__construct(false, $name = esc_html__('MT: Contact Info Widget', 'sensible-wp'), $widget_ops);
	...
	<p><label for="<?php echo sanitize_text_field( $this->get_field_id( 'address' )); ?>"><?php esc_html_e( 'Enter your address', 'sensible-wp' ); ?></label>
	...
	<p><label for="<?php echo sanitize_text_field( $this->get_field_id( 'phone' )); ?>"><?php esc_html_e( 'Enter your phone number', 'sensible-wp' ); ?></label>
	...
	<p><label for="<?php echo sanitize_text_field( $this->get_field_id( 'email' )); ?>"><?php esc_html_e( 'Enter your email address', 'sensible-wp' ); ?></label>
	...
	function widget($args, $instance) {
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : esc_html__( 'Contact info', 'sensible-wp' );
		...
		if( ($address) ) {
			...
			echo '<span>' . esc_html__('Address: ', 'sensible-wp') . '</span>' . $address;
		...
		if( ($phone) ) {
			...
			echo '<span>' . esc_html__('Phone: ', 'sensible-wp') . '</span>' . $phone;
		...
		if( ($email) ) {
			...
			echo '<span>' . esc_html__('Email: ', 'sensible-wp') . '</span>' . '<a href="mailto:' . $email . '">' . $email . '</a>';
